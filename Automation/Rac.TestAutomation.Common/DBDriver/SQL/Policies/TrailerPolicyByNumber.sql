/*
PURPOSE: Fetch Trailer Policy data for verification
AUTHOR: Troy Hall
...
from     p_policy p
    ...
    join  t_bm_level_raci tblr                   on tblr.id = asvr.ncb_level_id
...
where   p.external_policy_number = @policynumber  --For existing buffered Policy Number
    and tplo.external_code = 'ATRO'  --Trailer
    and ai.installment_number = 1 --First instalment only
    and cne.email_type = 1 -- Private email only
