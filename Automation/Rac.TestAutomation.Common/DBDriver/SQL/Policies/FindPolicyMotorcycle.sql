/*
PURPOSE: Find Motorcycle policies for duplicate alert testing.
Returns policy number, contact details, and motorcycle registration.
AUTHOR: AI Assistant
...
FROM p_policy p
    JOIN p_pol_header ph ON ph.active_policy_id = p.id
        ...
        AND ph.product_id = 1000032 -- MGC Motorcycle Product
...
WHERE p.ENDORSMENT_TYPE_ID != 10 -- Exclude policies in renewal (10 = Policy Renewal)
    AND p.policy_start_date > DATEADD(month, -1, GETDATE()) -- Policy started within last 1 month
    AND asi.value IS NOT NULL
    AND asi.value != ''
    AND UPPER(asi.value) != 'TBA'
    AND PATINDEX('%[A-Z]%', UPPER(asi.value)) > 0 -- Registration must contain at least one letter
