/*
PURPOSE: Find bank account, amount and date for an upcoming instalment on a policy.
...
WHERE ph.product_id             = @productId -- 1000001:  Home Policy
  ...
  AND (SELECT count(*) FROM CN_CONTACT_BANK_ACCOUNT ccba2 WHERE ccba2.CONTACT_ID = ccba.CONTACT_ID
       AND ccba.BANK_NAME not in ('AMEX', 'MASTERCARD', 'VISA') and ccba.DISCONTINUE_DATE is null) >= 1
