...
{
    public class ClaimMotorPage4OtherParties : BasePage
    {
        #region XPATHS
        ...
        private const string XP_THIRDPARTY_QAS_FIELD     = "id('WitnessDetails_ThirdPartyAddress_qasautocomplete')";
        private const string XP_THIRDPARTY_QAS_SUGGESTION= "id('WitnessDetails_ThirdPartyAddress')//table[@class='address-find-table']//tr/td[1]";
        ...
        public ClaimMotorPage4OtherParties(Browser browser) : base(browser)
        {
        }
        ...
        override public bool IsDisplayed()
        {
            ...
            try
            {
                ...
                Reporting.LogPageChange("Motor claim other parties details page");
                rendered = true;
            }
        ...
        public void EnterWitnessInformation(ClaimCar claimData)
        {
            ...
            Reporting.Log("Witness 1 details completed.", _browser.Driver.TakeSnapshot());
            ...
            if (witnessCount > 0)
            {
                ...
                Reporting.Log("Witness 2 details completed.", _browser.Driver.TakeSnapshot());
        ...
        public void EnterThirdPartyInformation(ClaimCar claimData)
        {
            SetThirdPartyDetailsTheft(claimData);

            Reporting.Log("Third party details completed.", _browser.Driver.TakeSnapshot());
            ClickControl(XP_THIRDPARTY_CONTINUE_BTN);
        }

        public void UploadDocumentationAndSubmitClaim(ClaimCar claimData)
        {
        ...
        {
            if (DataHelper.IsValidTitle(contact.Title))
            {
        ...
        private void SetThirdPartyDetailsTheft(ClaimCar claimData)
        {
            var thirdPartyCount = claimData.ThirdParty == null ? 0 : claimData.ThirdParty.Count;

            OffenderCount = thirdPartyCount;
            var contacts = claimData.ThirdParty;

            bool noThirdParty = contacts == null || contacts.Count == 0;
            IWebElement haveTPInfoPrompt;
            if (_driver.TryWaitForElementToBeVisible(By.XPath(XP_OFFENDER_DETAILS_YN), WaitTimes.T5SEC, out haveTPInfoPrompt))
            {
                ClickBinaryToggle(XP_OFFENDER_DETAILS_YN, XPR_NO, XPR_YES, noThirdParty);
            }
            ...
            SetThirdPartyDetailsCommon(contact);

            if (!string.IsNullOrEmpty(contact.AdditionalInfo))
                WaitForTextFieldAndEnterText(XP_OFFENDER_RELATIONSHIP, contact.AdditionalInfo, false);
        }

        private void SetThirdPartyDetailsCommon(ContactClaimMotorTP contact)
        {
            ...
            {
                QASSearchForAddress(XP_THIRDPARTY_QAS_FIELD, XP_THIRDPARTY_QAS_SUGGESTION, contact.MailingAddress.StreetSuburbState());
            }
        }
